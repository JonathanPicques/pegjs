...
Postgrest
    = p:PostgrestLogical { return `${p.name}=(${p.args})`; }
    / p:PostgrestFunction { return `${p.column}=${p.name}.${p.value}`; }
    / p:PostgrestComparison { return `${p.column}=${p.name}.${p.value}`; }

PostgrestNot
    = "NOT"i "(" p:(PostgrestLogical) ")" { return `not.${p.name}(${p.args})`; }
    / "NOT"i "(" p:(PostgrestFunction / PostgrestComparison) ")" { return `${p.column}.not.${p.name}.${p.value}`; }
    
PostgrestLogical
    = name:("OR"i / "AND"i) "(" __ args:PostgrestLogicalItem __ ")" { return {name: name.toLowerCase(), args}; }
...
PostgrestLogicalItem
    = head:PostgrestLogicalArg __ "," __ tail:PostgrestLogicalItem { return [head, ...tail]; }
...
PostgrestColumnName "column"
    = name:PostgrestColumnIdentifierName { return encodeURI(name); }

PostgrestColumnIdentifierName
